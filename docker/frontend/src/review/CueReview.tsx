...
export function CueReview({ item, disabled, onMutation }: { item: ReviewItem; disabled: boolean; onMutation: (pending: boolean, message?: string) => void }) {
  ...
  useEffect(() => {
    ...
    void requestJson<CueReviewPayload>(`/api/manual-reviews/${item.id}/cues?page=${page}&pageSize=1`, {}, controller.signal)
      .then((value) => { if (!controller.signal.aborted) { setData(value); setPage(value.pagination.page); setStale(false); setRemember(Boolean(value.items[0]?.rememberPhrase)); } })
  ...
  const cue = data?.items?.[0], currentPage = data?.pagination.page || page;
  const navigationBlocked = pending || loading || stale;
  const mutationBlocked = disabled || navigationBlocked || !data?.actionsEnabled;
  const revocationBlocked = disabled || pending || loading || stale;
  ...
  return <section className="cue-review" aria-label="Cue review workspace" aria-busy={loading || pending}>
    <div className="cue-review-heading"><div><p className="eyebrow">Review cues</p><h3>{item.media?.episodeCode || "Media item"} · {data?.sourceLanguage || "—"} → {data?.targetLanguage || item.targetLanguage || "—"}</h3></div>{Boolean(data?.pagination.total) && <span className="badge badge-warning">Issue {currentPage} of {data?.pagination.total}</span>}</div>
    ...
    {cue && <article className="cue-comparison"><header><h4>Cue {cue.cueNumber}</h4><time>{cue.timestamp}</time></header>
      ...
      {copied && <label className="remember-choice"><input type="checkbox" checked={remember} onChange={(event) => setRemember(event.target.checked)} disabled={mutationBlocked || cue.decision === "retry"} /> Remember this exact phrase for {data.scope.startsWith("sonarr:") ? "this series" : "this item"} ({data.scope}), {data.sourceLanguage} → {data.targetLanguage}</label>}
      <div className="cue-decision-actions">{cue.decision ? <><span className={`badge ${cue.decision === "approve" ? "badge-success" : "badge-warning"}`}>{cue.decision === "approve" ? "Approved" : "Try again selected"}</span><button className="btn btn-sm btn-secondary" disabled={mutationBlocked} onClick={() => void act("clear_cue_decision", cue)}>Undo decision</button></> : <><button className="btn btn-sm btn-primary" disabled={mutationBlocked || !cue.canApproveCue} title={!cue.canApproveCue ? "This cue has a mandatory finding" : undefined} onClick={() => void act("approve_cue", cue)}>Approve cue</button><button className="btn btn-sm btn-secondary" disabled={mutationBlocked} onClick={() => void act("retry_cue", cue)}>Try again</button></>}</div>
    </article>}
    {data?.candidateAvailable && data.pagination.total === 0 && !data.fileFindings?.length && <p>No unresolved cue findings in this candidate.</p>}
    {Boolean(data?.pagination.total) && <nav className="review-pagination" aria-label="Cue navigation"><button className="btn btn-sm btn-secondary" disabled={navigationBlocked || currentPage <= 1} onClick={() => setPage(currentPage - 1)}>Previous issue</button><span>{currentPage} of {data?.pagination.total}</span><button className="btn btn-sm btn-secondary" disabled={navigationBlocked || currentPage >= (data?.pagination.total || 0)} onClick={() => setPage(currentPage + 1)}>Next issue</button></nav>}
    {data?.pagination && <><div className="review-finish"><p><strong>{data.decisionCounts?.approved || 0}</strong> approved · <strong>{data.decisionCounts?.retry || 0}</strong> try again · <strong>{data.decisionCounts?.undecided ?? data.pagination.total}</strong> undecided</p><button className="btn btn-primary" disabled={mutationBlocked || Boolean(data.fileFindings?.length) || (data.decisionCounts?.undecided ?? data.pagination.total) > 0} onClick={() => void act("finish_review")}>Finish review</button></div>
      <details><summary>Remembered phrases</summary>{!data.approvals.length && <p>No remembered phrases.</p>}{data.approvals.map((approval) => <div className="remembered-phrase" key={approval.id}><span>{approval.sourceText} → {approval.targetText}</span><button className="btn btn-sm btn-secondary" disabled={revocationBlocked} onClick={() => void act("revoke_name", undefined, approval.id)}>Forget approval</button></div>)}</details></>}
  </section>;
}
