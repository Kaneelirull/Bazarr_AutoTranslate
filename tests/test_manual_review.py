...
class ManualReviewTests(unittest.TestCase):
    ...
    def test_review_assets_are_semantic_responsive_and_preserve_filter_focus(self):
        script = (REPO_ROOT / "docker" / "static" / "review.js").read_text(encoding="utf-8")
        stylesheet = (REPO_ROOT / "docker" / "static" / "dashboard.css").read_text(encoding="utf-8")
        self.assertIn('href="/review" aria-current="page"', script)
        self.assertIn('class="review-detail-row"><td colspan="6">', script)
        self.assertIn('data-review-details=', script)
        self.assertIn("expandedReviewIds", script)
        self.assertIn('X-Bazarr-Autotranslate-Action', script)
        self.assertIn("captureFocus", script)
        self.assertIn("filterState = filterValues()", script)
        self.assertIn('viewState = payload ? "refresh-error"', script)
        self.assertIn('if (viewState === "initial-error")', script)
        self.assertIn('id="review-action-status"', script)
        self.assertIn('disabled title="Manual actions are disabled by configuration"', script)
        self.assertIn("completenessDetails", script)
        self.assertIn("mediaRelativePath", script)
        self.assertIn("confirm(`Are you sure", script)
        self.assertIn("No manual reviews match these filters.", script)
        self.assertIn(".review-summary", stylesheet)
        self.assertIn(".review-table td.cell-actions", stylesheet)
        self.assertIn("overflow-x: auto", stylesheet)
        self.assertIn("cursor: not-allowed", stylesheet)
        self.assertIn('.btn[aria-busy="true"]', stylesheet)
        self.assertIn("@media (max-width: 920px)", stylesheet)
        self.assertIn("@media (max-width: 440px)", stylesheet)
        narrow_rules = stylesheet.split("@media (max-width: 440px)", 1)[1]
        self.assertNotIn(".review-summary,\n  .review-filters", narrow_rules)
        self.assertIn("sourceAvailabilityReason", script)
        self.assertIn("targetAvailabilityReason", script)
        self.assertIn("artifactAvailabilityReason", script)
        self.assertIn("Clear filters", script)
        self.assertIn('role="${actionError ? "alert" : "status"}"', script)
        self.assertIn("timeZone: configuredTimeZone", script)
        self.assertIn("Technical codes", script)
        self.assertIn("Retries completed", script)
        self.assertIn("min-height: 44px", stylesheet)
        self.assertIn(".review-managed-path", stylesheet)
        ...
        self.assertNotIn("min-width: 320px", stylesheet)
        self.assertNotIn(".retry-table .cell-details {\n  width: 1%;", stylesheet)
